...
const Page = () => {
  ...
  const openEdit = (item: StatusCode) => {
  ...
  const openDelete = (item: StatusCode) => {
    setSelectedItem(item)
  ...
  const handleSave = async () => {
    ...
    try {
      const companyId = getCompanyId()
      if (selectedItem) {
        await statusCodeService.update(companyId, selectedItem.id, { description: form.description })
      } else {
        await statusCodeService.create(companyId, { id: form.id, description: form.description })
      }
      setShowModal(false)
      await fetchData()
    } catch (err: any) {
    ...
    <Container fluid className="py-3">
      <div className="card shadow-sm">
        <div className="card-header d-flex justify-content-between align-items-center py-2">
          <h6 className="mb-0 fw-semibold d-flex align-items-center gap-2">
            <LuTag /> Status Codes ({totalCount})
          </h6>
          <Button size="sm" variant="primary" onClick={openCreate}>
            <LuPlus className="me-1" />Add Status Code
          </Button>
        </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>{selectedItem ? 'Edit Status Code' : 'Add Status Code'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <FormGroup className="mb-3">
            ...
            <FormControl size="sm" type="text" placeholder="e.g. Available"
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered size="sm">
        <Modal.Header closeButton>
          <Modal.Title>Delete Status Code</Modal.Title>
        </Modal.Header>
        <Modal.Body>
