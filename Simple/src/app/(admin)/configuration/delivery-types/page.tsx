...
import { LuPlus, LuPencil, LuTrash2, LuFileText } from 'react-icons/lu'
...
interface DeliveryTypeForm { id: number; description: string }
...
const Page = () => {
  ...
  const openEdit   = (item: DeliveryType) => { setSelectedItem(item); setForm({ id: item.id, description: item.description }); setError(null); setShowModal(true) }
  const openDelete = (item: DeliveryType) => { setSelectedItem(item); setShowDeleteModal(true) }
  ...
  return (
    <Container fluid className="py-3">
      <div className="card shadow-sm">
        <div className="card-header d-flex justify-content-between align-items-center py-2">
          <h6 className="mb-0 fw-semibold d-flex align-items-center gap-2">
            <LuFileText size={15} /> Delivery Types ({totalCount})
          </h6>
          <Button size="sm" onClick={openCreate}
            className="flex items-center gap-1.5 btn-navy rounded-md">
            <LuPlus size={13} /> Add Delivery Type
          </Button>
        </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        <Modal.Header closeButton className="modal-header-dark">
          <Modal.Title style={{ fontSize: 15, fontWeight: 600 }}>
            {selectedItem ? 'Edit Delivery Type' : 'Add Delivery Type'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input className="dash-filter-input" placeholder="e.g. Standard Delivery"
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered size="sm">
        <Modal.Header closeButton className="modal-header-dark">
          <Modal.Title style={{ fontSize: 15, fontWeight: 600 }}>Delete Delivery Type</Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
